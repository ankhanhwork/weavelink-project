...
| Mockup image | img/S43-analytics_dashboard_screen.png — historical layout reference only |
| Status | Product choices confirmed; runtime configuration follows Plan |

## 1. Purpose

Sales Admin views Dony's business performance and conversion/waiting evidence, asks supported questions and exports the selected result. Customer-behavior analysis is explicitly labeled Authenticated customers only; the dashboard does not claim to include guest browsing. Keep the internal CRM shell and existing visual identity. Customer, Sales and System Admin cannot access this screen through navigation, direct URL, API, AI result or export links.
...
## 2. Historical visual reference

![Historical visual reference](img/S43-analytics_dashboard_screen.png)

Written behavior is authoritative. The historical image does not represent the new tabs, AI panel, conversion rules or current export policy. Keep its CRM visual identity only; do not copy its completed-order count label as the all-created-order KPI, add design fees twice, stack revenue plus cash as one total, or use its obsolete 24-hour link lifetime. No new demo/mockup is required by this documentation change.

## 3. Element inventory and layout

| Element | Content / behavior | Validation |
| --- | --- | --- |
| Header and tabs | Dashboard title, two tabs, Analyze with AI, export action | Sales Admin; selected tab and filters survive authorized back navigation |
| Shared filters | Local date range, product, supported buyer segment/organization; explicit Apply | Default last 30 days inside the rolling last 12 calendar months, inclusive start/exclusive end, max span 366 days; server returns available_start/available_end_exclusive per MFG-11 5.7; historical archived products remain selectable; organization is not tenancy |
...
| Overview KPI row | Recognized revenue, cash collected, created orders, new customers; cancellation count separately | Exact MFG-11 BR-001/002; fee revenue is included component, not a fifth amount added to revenue |
...
| Overview supporting orders | Authorized records for the declared metric/date basis, with pagination | Explain created_at versus completed_at basis; navigate to S29, never fabricate reconciliation |
...
| Funnel summary | Full entry count, eligible/immature counts, final-stage conversion, explicit cancellations and separately scoped current waiting summary | Label product entries, intents and orders separately; do not sum child-order states as exclusive entry/intent states |
...
| Step detail | Waiting-state breakdown, responsible actor/source, completed-transition median with sample size, open-wait age and revision-cycle detail | Distinguish sample transit, revision, Dony contract preparation and customer signing; no invented SLA or inactivity threshold |
...
| Export | Dataset and CSV/XLSX format, queue/status/download | Shared result snapshot; allowlisted business/funnel aggregates, 100,000-row cap, private URL lasting at most 10 minutes, capped by file expiry |
...
1. Opening S43 defaults to Overview and the last 30 local days. Expose the date basis and refreshed time before the user interprets values.
2. Switching to Journey & conversion preserves compatible filters and explicitly shows entry-cohort semantics and observation cutoff. Unsupported dimensions require a clear correction, not silently dropped filters.
...
5. Analyze with AI opens the panel with visible current context. Suggested questions include “Which stage has the lowest observed conversion?”, “Where are orders currently waiting?” and “Why is cash collected different from revenue?”
...
| SR-006 | Historical screenshot and illustrative demo figures are not requirements or data sources |

Acceptance scenarios:

1. Deposit changes accepted cash but not recognized revenue; Completed and recognized refunds drive revenue; design fees are not added twice.
...
- [x] Route, actor, release target and historical-image limitations are explicit.
