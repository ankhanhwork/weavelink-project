...
| Success | Refresh S41 Dony Staff Accounts from the committed server response, expose only the next role/state-allowed action and announce the result via aria-live. | Mutation commits |
| Conflict | For a stale S41 Dony Staff Accounts version or lifecycle state, reload authoritative data, explain the conflict and require explicit review before resubmission. | 409 |
...
|---|---|
| MFG-03/F-ACC-001 | UI touchpoint for **Staff Account List View**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-03/F-ACC-002 | UI touchpoint for **Add Staff Form**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-03/F-ACC-003 | UI touchpoint for **Generate Staff Invitation**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-03/F-ACC-004 | UI touchpoint for **Save Staff Account Logic**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-03/F-ACC-005 | UI touchpoint for **Staff Account Detail View**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-03/F-ACC-006 | UI touchpoint for **Update Staff Account Logic**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-03/F-ACC-007 | UI touchpoint for **Confirm Staff Removal UI**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-03/F-ACC-008 | UI touchpoint for **Soft-delete Staff Account**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
