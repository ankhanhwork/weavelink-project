...
| Empty | Render the defined initial/empty state for S02 Sign Up; if a required route object is absent, return a safe 404 and the authorized parent route. | Empty initial form or missing detail payload |
...
| Success | Refresh S02 Sign Up from the committed server response, expose only the next role/state-allowed action and announce the result via aria-live. | Mutation commits |
| Conflict | For a stale S02 Sign Up version or lifecycle state, reload authoritative data, explain the conflict and require explicit review before resubmission. | 409 |
...
|---|---|
| MFG-01/F-USER-001 | UI touchpoint for **Registration Screen**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-01/F-USER-002 | UI touchpoint for **Registration Logic**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
| MFG-01/F-USER-003 | UI touchpoint for **Send email verification link**; this screen defines the visible action/result, while the module spec owns server authorization, validation and persistence. |
