...
const s34 = read('screens/S34-contract_detail_screen_customer.md');
const s44 = read('screens/S44-staff_login_screen.md');
...
check(readme.includes('Sales Admin') && readme.includes('lands on S28') && readme.includes('Sales and System Admin') && readme.includes('land on public S01'), 'README: MVP staff login landing routes must be defined');
check(readme.includes('MFG-02 profile/settings screens') && /hide the Profile\/S06 link/i.test(readme), 'README: MFG-02 MVP exclusion and Profile navigation visibility must be explicit');
...
check(s35.includes('MVP is standard-order only: always 0 because `merge_opt_in=false`'), 'S35: MVP merge discount must be explicitly zero');
check(s34.includes('MVP does not require password re-entry or a one-time signing challenge') && s34.includes('active Customer session is the authentication evidence'), 'S34: MVP contract acknowledgement authentication level must be explicit');
check(s44.includes('Post-MVP token-bound state') && s44.includes('Post-MVP only; unavailable to pre-provisioned MVP accounts'), 'S44: invitation acceptance must be marked post-MVP in both inventory and interactions');
