...
| BR-007 | Delivery proof is either a carrier tracking event/status showing delivered, independently checked by Sales Admin, or a signed proof-of-delivery asset/reference recorded by Sales Admin with delivered timestamp. If the Customer has not disputed or confirmed receipt within 3 calendar days of that verified timestamp, a scheduled event records `received_at` and `DeliveredAwaitingBalance`; the Customer is notified before and after auto-confirmation. A customer dispute pauses auto-confirmation and requires Sales Admin review. `Shipped` alone never enables BALANCE payment. | Avoid orders being stuck indefinitely while retaining a dispute window. |
...
| --- | --- | --- |
| Quote | id, customer_id, buyer_type, buyer_organization_snapshot?, design/product IDs and versions, quantities, address, merge/policy, merchandise subtotal, shipping, tax, design fee, total, deposit preview, expiry, version | Server-computed, customer-owned and consumed once. Organization data is a commercial snapshot, never authorization or tenant scope. |
| BuyerOrganization snapshot | buyer_type, legal_name?, tax_id?, billing_address? | Captured on S22 for Business Buyer/Reseller Shop; required there and copied immutably to Quote, Order and Contract. It is customer-provided commercial context, not a login, role, tenant or authorization boundary. Individual orders omit it. |
| Order | id, customer_id, buyer_type, buyer_organization_snapshot?, design/product/quote versions, status, price/address/quantity snapshots, approved_sample_id?, contract_id?, deposit_percent, contract_total_vnd, accepted_deposit_vnd, accepted_order_credit_vnd, balance_due_vnd, balance_due_at, received_at?, version, timestamps | Made-to-order aggregate linked to sample, contract, two payment purposes, fulfilment and optional merge batch. |
